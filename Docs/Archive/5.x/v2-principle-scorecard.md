...
| Child-flow result handoff | `ChildCoordinator.waitForResult() async -> Result?` gives any app-defined flow owner a structured result handoff with caller-task cancellation, without requiring `Coordinator` conformance | `Sources/InnoRouterSwiftUI/ChildCoordinator.swift`, `Docs/design-child-coordinator-handoff.md` | Enforced |
...
| Macro diagnostics + FixIts | `@Router` diagnoses invalid attachment, destination shape, generated-witness conflicts, empty routers, and redundant route conformances. `@Routable` and `@CasePathable` share enum-shape diagnostics and Fix-Its for supported declaration kinds. | `Sources/InnoRouterMacrosPlugin/{RouterMacroDiagnostic,MacroDiagnostic}.swift` | Enforced |
| Responsibility budgets | Production files, types, functions, and branch complexity have strict budgets independent of large test fixtures. | `.swiftlint.sources.yml`, `scripts/lint-source-gates.sh` | Enforced |
...
- App-defined flow owners await child finish values through structured `ChildCoordinator.waitForResult() async -> Result?` without conforming to `Coordinator` or hand-rolling continuation plumbing. The method waits for a result but never presents the child; app-owned state remains responsible for placement. Cancelling its caller task propagates to the child through `ChildCoordinator.parentDidCancel()` (default no-op), resolves with `nil`, and lets transient child state tear down cleanly when the owner disappears.
