...
struct FlowModelState: Equatable {
    ...
    private mutating func applyPushMany(
        ...
        let command: NavigationCommand<PropertyRoute> = routes.count == 1
            ? .push(routes[0])
            : .pushAll(routes)
        switch navigationDecision(for: command, middlewarePolicy: middlewarePolicy) {
        case .cancel(let debugName, _):
            return reject(.middlewareRejected(debugName: debugName))
        case .proceed(let command):
            let changed = applyNavigationCommand(command)
    ...
    private mutating func applyPopCount(
        ...
        let command: NavigationCommand<PropertyRoute> = count > 0 && count == navigationPath.count
            ? .popToRoot
            : .popCount(count)
        switch navigationDecision(for: command, middlewarePolicy: middlewarePolicy) {
        case .cancel(let debugName, _):
