# NavigationStore and hosts

@Metadata {
  @PageKind(article)
}

`RouterHost` is the default self-contained stack-plus-modal surface.
`NavigationStore` is the advanced stack-only authority to use when the
application needs to own navigation state outside a host.

## Start macro-first

For a local stack and modal flow, declare destinations on the route and let
`RouterHost` own the unified store:

```swift skip doc-fragment
@Router
enum AppRoute {
    case detail(id: String)
    case settings

    var destination: some View {
        switch self {
        case .detail(let id):
            DetailView(id: id)
        case .settings:
            SettingsView()
        }
    }
}

struct AppRoot: View {
    var body: some View {
        RouterHost(AppRoute.self) {
            HomeView()
        }
    }
}

struct HomeView: View {
    @EnvironmentRouter(AppRoute.self) private var router

    var body: some View {
        Button("Open settings") {
            router.go(.settings)
        }
    }
}
```

`@Router` generates `Route` and `DestinationRoute` conformance. If the host
is missing or the route type does not match the nearest host,
`@EnvironmentRouter` follows `EnvironmentMissingPolicy` and fails loudly by
default.

Add literal origin allowlists and `@DeepLink` markers when a URL resolves to
one pushed route. `RouterHost` installs the incoming-URL handler automatically;
no application `onOpenURL` bridge is needed for that path.

## Promote the store when another boundary needs it

Create the `NavigationStore` outside the host when authentication or pending
replay, multi-step deep-link reconciliation, state restoration, middleware
mutation, event observation, or another application boundary needs direct
access. The `DestinationRoute` generated by `@Router` keeps this advanced host
concise:

```swift skip doc-fragment
@State private var navigation = NavigationStore<AppRoute>()

NavigationHost(store: navigation) {
    HomeView()
}
```

## What the store owns

`NavigationStore` owns:

- the current `RouteStack` snapshot
- command execution
- batch execution
- transactional execution
- path reconciliation from `NavigationStack(path:)`
- middleware registry orchestration
- typed observation events for analytics and diagnostics

Configuration is centralized in `NavigationStoreConfiguration`.
Its synchronous observation surface is one `onEvent` callback that
switches over `NavigationEvent`; `NavigationStore.events` provides the
same cases as an `AsyncStream`.

```swift skip doc-fragment
let configuration = NavigationStoreConfiguration<AppRoute>(
    onEvent: { event in
        switch event {
        case .changed(_, let new):
            analytics.recordPath(new.path)
        case .batchExecuted(let result):
            analytics.recordBatch(result)
        case .transactionExecuted(let result):
            analytics.recordTransaction(result)
        case .middlewareMutation(let mutation):
            diagnostics.record(mutation)
        case .pathMismatch(let mismatch):
            diagnostics.record(mismatch)
        }
    }
)
```

For a 5.0 migration, move the bodies of the former per-event
configuration closures into these switch cases. No compatibility
closures remain.

## Host responsibilities

`NavigationHost` and `CoordinatorHost` connect SwiftUI navigation UI to the store or coordinator:

- render the root view
- render destinations for pushed routes
- publish one route-typed authority consumed through `@EnvironmentRouter`
- bridge system path changes back into router commands

Views should not mutate a store directly. The default view-facing surface is
`@EnvironmentRouter`, whose `RouterActions` methods emit typed
`NavigationIntent` values. Use `go`, `back`, and the other focused conveniences
for common transitions, and `send(_:)` as the explicit escape hatch for an
advanced intent. The same `@EnvironmentRouter(Route.self)` surface is used for
navigation, modal, flow, and tab capabilities supplied by the matching host.

Use `onEvent` for synchronous analytics or diagnostics and `events` when an
asynchronous consumer should own the observation task. Supplying `logger`
also records private, structured summaries through the store's internal OSLog
integration; no separate telemetry adapter is required.

### Intent command shape

`NavigationStore.send(_:)` and the navigation adapter owned by `FlowStore`
use the same middleware-visible command shape:

- empty `goMany` / `pushMany` -> no command
- one route -> `.push`
- multiple routes -> one atomic `.pushAll`
- `backBy` / `popCount` equal to the current stack depth -> `.popToRoot`

With no modal tail, pop attempts enter middleware even on an empty stack.
The engine may still return a no-op or failure, but middleware can consistently
observe, cancel, or rewrite the attempt. A modal tail remains the flow boundary:
stack-pop intents do not bypass it.

## Path reconciliation

When SwiftUI changes the path, InnoRouter translates that change back into semantic commands:

- prefix shrink -> `.popCount` or `.popToRoot`
- prefix expand -> per-step `.push` batch
- mismatch -> `NavigationPathMismatchPolicy`

This preserves command meaning instead of treating every path change as a blind replacement.

`NavigationStoreConfiguration` defaults to `.replace`, which treats the
SwiftUI binding as the source of truth for a non-prefix rewrite and emits
a `NavigationPathMismatchEvent`. For debug or pre-release builds, use
`.assertAndReplace` to make unexpected rewrites loud while still
recovering. Use `.ignore` only when the store must remain the sole
authority, and `.custom` when a host has domain-specific repair rules.
