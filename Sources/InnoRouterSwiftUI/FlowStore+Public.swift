...
extension FlowStore {
    ...
    /// sink, it returns `.rejected(currentPath:reason:)` with
    /// `.reentrantApply` without mutating state for that apply call. The
    /// returned path is a snapshot captured at the rejection point.
